...
def filter_podcast_highlights(books, highlights):
    ...
    podcast_book_ids = {book["id"] for book in books if book.get("category") == "podcasts"}
    return [h for h in highlights if h["book_id"] in podcast_book_ids and "Transcript:" in h["text"]]
